...
5. If a slot is free: choose a hypothesis from a different family than the tests already running, write one ledger entry, change that one slot's config (and bot/strategy.py only if new logic is needed, with tests), run `python -m pytest tests -q` and `python -m bot.backtest --config configs/challenger<k>.yaml`, put the metrics in the ledger entry, remove the idea from the backlog, and commit with first line `H<n>: <title>`.
